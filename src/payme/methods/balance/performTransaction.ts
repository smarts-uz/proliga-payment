...
export async function performTransaction(
  this: any,
  BalancePerformTransactionDto: BalancePerformTransactionDto,
) {
  const transaction = await this.prismaService.pay_balance.findUnique({
    where: { transaction_id: BalancePerformTransactionDto.params.id },
  });
  ...
  if (transaction.state !== TransactionState.Pending) {
    if (transaction.state !== TransactionState.Paid) {
      ...
      return {
        ...
        id: BalancePerformTransactionDto.params.id,
      };
    }
